...
export default function SearchPlace({ closeModal }: SearchPlaceProps) {
  ...
  return (
    <div className="h-full relative overflow-hidden sm:h-118">
      <div
        className={`px-5 h-full pb-30 sm:border-b-2 sm:border-primary absolute inset-0 overflow-hidden transform-all duration-700 ease-in-out ${selected ? '-translate-x-full pointer-events-none' : 'translate-x-0'}`}
      >
        <SearchInputField
          value={searchPlace}
      ...
      >
        <div className="flex flex-col justify-between h-full">
          ...
          <Button onClickFunc={onClickAddPlace}>{selected?.place_name} 추가하기</Button>
        </div>
      </div>
    </div>
  );
